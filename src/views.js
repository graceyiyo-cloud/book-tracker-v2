...
const status = (b) => {
  const state = kind(b);
  return `<span class="status ${state}">${labels[state]}</span>`;
};
const stars = (rating) => {
...
function bookRow(b, i, m) {
  ...
  const recommendation = b.reviewUrl
    ? `<div class="book-links book-recommendation">${ico("like")}<div>${linkedText(b.reviewUrl)}</div></div>`
    : "";
  const synopsisToggle = b.synopsis
    ? `<button class="synopsis-toggle" data-act="synopsis" data-id="${esc(b.id)}" aria-expanded="${expanded}">${ico("book")}${expanded ? "收合大綱" : "大綱"}</button>`
    : "";
  const primaryAction =
    b.status === "reading"
      ? `<button class="book-action" data-act="${b.isReadingNow ? "finish" : "toggleReading"}" data-id="${esc(b.id)}">${b.isReadingNow ? "記錄完食" : "開始閱讀"}${ico("arrow")}</button>`
      : "";
  const record =
    b.status === "finished"
      ? `<div class="record-summary"><span>完食於 ${esc(b.finishDate || "未填日期")}</span>${iconButton("finish", "修改完食紀錄", "edit", b.id)}${b.review ? `<p>${esc(b.review)}</p>` : ""}</div>`
      : b.status === "dropped"
        ? `<div class="record-summary dropped"><span>${esc(b.dropDate || "未填日期")} · 第 ${esc(b.dropChapter || "?")} 章棄書</span>${iconButton("drop", "修改棄書紀錄", "edit", b.id)}${b.dropReason ? `<p>${esc(b.dropReason)}</p>` : ""}</div>`
        : "";
  const synopsisText =
    expanded && b.synopsis
      ? `<div class="synopsis-text prose">${esc(b.synopsis)}</div>`
      : "";
  return `<article class="book${b.isPinned ? " is-pinned" : ""}${b.isReadingNow ? " is-reading-now" : ""}${b.reviewUrl ? " has-recommendation" : ""}" data-book="${esc(b.id)}"><div class="book-marker"><span class="book-index">${String(i + 1).padStart(2, "0")}</span>${b.isPinned ? `<span class="pin-mark" role="img" aria-label="置頂" title="置頂">${ico("pin")}</span>` : ""}</div><div class="book-body"><a class="book-title" href="${esc(searchUrl(b.title, b.author))}" target="_blank" rel="noopener noreferrer" title="搜尋書名與作者">${esc(b.title)} ${ico("link")}</a><div class="meta"><button class="meta-link" data-act="quickSearch" data-value="${esc(b.author || "佚名")}">${esc(b.author || "佚名")}</button><span>${wordText(b)}</span><span>${esc(dateValue(b) || "")}</span></div></div><div class="book-side"><div class="book-badges">${status(b)}${iconButton("detail", `查看《${b.title}》詳情`, "book", b.id)}</div></div><div class="tags book-tags">${tags(b)}${stars(b.rating)}</div>${recommendation}<div class="book-footer">${synopsisToggle}<div class="row-actions">${primaryAction}${iconButton("menu", `《${b.title}》更多操作`, "more", b.id)}</div></div><div class="book-notes">${record}${synopsisText}</div></article>`;
}
function nav(m, cls) {
